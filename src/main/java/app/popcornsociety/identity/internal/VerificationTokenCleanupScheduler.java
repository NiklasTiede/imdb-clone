...
public class VerificationTokenCleanupScheduler {
  ...
  public VerificationTokenCleanupScheduler(
      JdbcTemplate jdbc,
      @Value("${popcorn-society.identity.tokens.retention-days:30}") int retentionDays,
      @Value("${popcorn-society.retention.batch-size:1000}") int batchSize) {
    Assert.isTrue(
        retentionDays > 0, "popcorn-society.identity.tokens.retention-days must be positive");
    Assert.isTrue(
