...
public class SecurityAuditEventCleanupScheduler {
  ...
  public SecurityAuditEventCleanupScheduler(
      JdbcTemplate jdbc,
      @Value("${popcorn-society.identity.audit.retention-days:90}") int retentionDays,
      @Value("${popcorn-society.retention.batch-size:1000}") int batchSize) {
    Assert.isTrue(
        retentionDays > 0, "popcorn-society.identity.audit.retention-days must be positive");
    Assert.isTrue(
