package app.popcornsociety.shared.internal.observability;

import app.popcornsociety.shared.api.FrontendTelemetryBatch;
import app.popcornsociety.shared.api.FrontendTelemetryEvent;
import app.popcornsociety.shared.api.FrontendTelemetryName;
...
public class FrontendTelemetryMetrics {
  ...
  private static final AttributeKey<String> UI_ACTION_ATTRIBUTE =
      AttributeKey.stringKey("popcorn_society.frontend.ui_action.type");
  private static final AttributeKey<String> UI_ACTION_OUTCOME_ATTRIBUTE =
      AttributeKey.stringKey("popcorn_society.frontend.ui_action.outcome");
  ...
  private void record(FrontendTelemetryEvent event) {
    meterRegistry
        .counter("popcorn_society.frontend.events.accepted", "type", tag(event.type()))
        .increment();

    switch (event.type()) {
      case WEB_VITAL -> recordWebVital(event);
      case APP_BOOT -> recordDuration("popcorn_society.frontend.app.boot.duration", event.value());
      case ROUTE_NAVIGATION ->
          recordDuration("popcorn_society.frontend.route.navigation.duration", event.value());
      case API_REQUEST -> recordApiRequest(event);
      case BROWSER_ERROR ->
          Counter.builder("popcorn_society.frontend.browser.errors")
  ...
  private void recordUiAction(FrontendTelemetryEvent event) {
    ...
    Counter.builder("popcorn_society.frontend.ui.actions")
    ...
    Span.current()
        .addEvent(
            "popcorn_society.frontend.ui_action",
  ...
  private void recordWebVital(FrontendTelemetryEvent event) {
    if (event.name() == FrontendTelemetryName.CLS) {
      DistributionSummary.builder("popcorn_society.frontend.web.vital.cls")
    ...
    Timer.builder("popcorn_society.frontend.web.vital.duration")
  ...
  private void recordApiRequest(FrontendTelemetryEvent event) {
    Timer.builder("popcorn_society.frontend.api.request.duration")
