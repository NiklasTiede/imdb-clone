...
public class MovieSearchReindexRetention {
  ...
  public MovieSearchReindexRetention(
      JdbcTemplate jdbc,
      @Value("${popcorn-society.catalog.reindex.retention-days:30}") int retentionDays,
      @Value("${popcorn-society.retention.batch-size:1000}") int batchSize) {
    Assert.isTrue(
        retentionDays > 0, "popcorn-society.catalog.reindex.retention-days must be positive");
    Assert.isTrue(
