package app.popcornsociety.catalog.internal.search.query;
...
public class MovieSearchQueryBuilder {
                ...
                query.knn(
                    knn -> {
                      ...
                      if (!filters.isEmpty()) {
                        knn.filter(
                            Query.of(
                                filterQuery -> filterQuery.bool(bool -> bool.filter(filters))));
                      }
                      return knn;
                    }));

    return SearchRequest.of(search -> search.index(index).from(from).size(size).query(knnQuery));
  }
  ...
  public SearchRequest buildRecommendationCandidateSearchRequest(
    ...
    int numCandidates = Math.max(candidateLimit * NUM_CANDIDATES_MULTIPLIER, MIN_NUM_CANDIDATES);
    Query excludeAnchor = Query.of(query -> query.ids(ids -> ids.values(anchorMovieId.toString())));
                ...
                query.knn(
                    knn ->
                        knn.field(EMBEDDING_FIELD)
                            ...
                            .filter(
                                Query.of(
                                    filter -> filter.bool(bool -> bool.mustNot(excludeAnchor))))));

    return SearchRequest.of(
        search -> search.index(index).from(0).size(candidateLimit).query(candidateQuery));
  }
    ...
    return SearchRequest.of(
        search ->
            search
                ...
                .sort(
                    sort ->
                        sort.field(field -> field.field("imdbRatingCount").order(SortOrder.Desc)))
      ...
      filters.add(
          Query.of(
              query ->
                  query.range(
                      range ->
                          range.field("imdbRating").gte(JsonData.of(criteria.minImdbRating())))));
    }
      ...
      filters.add(
          Query.of(
              query ->
                  query.range(
                      range ->
                          range.field("rating").gte(JsonData.of(criteria.minCommunityRating())))));
    }
  ...
  private Query buildPopularityQuery() {
    return Query.of(
        q ->
            q.functionScore(
                fs ->
                    fs.query(matchAll -> matchAll.matchAll(all -> all))
                        .functions(
                            f ->
                                f.fieldValueFactor(
                                    fieldValueFactor ->
