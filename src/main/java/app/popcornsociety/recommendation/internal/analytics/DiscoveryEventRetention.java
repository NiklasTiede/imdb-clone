...
public class DiscoveryEventRetention {
  ...
  public DiscoveryEventRetention(
      JdbcTemplate jdbc,
      @Value("${popcorn-society.recommendation.discovery.retention-days:90}") int retentionDays,
      @Value("${popcorn-society.retention.batch-size:1000}") int batchSize) {
    Assert.isTrue(
        retentionDays > 0,
        "popcorn-society.recommendation.discovery.retention-days must be positive");
    Assert.isTrue(
