...
def test_metrics_expose_identity_and_bounded_http_labels(client: TestClient) -> None:
    ...
    assert "popcorn_society_agent_build_info" in response.text
    assert 'popcorn_society_agent_guardrail_limit{limit="max_concurrent_runs"} 2.0' in response.text
    assert "popcorn_society_agent_process_budget_committed_usd 0.0" in response.text
    assert (
        'popcorn_society_agent_http_requests_total{method="GET",route="/healthz",status="200"} 1.0'
        in response.text
    )
